...
// Import the data from the respective category pages
import { bedsheetsProducts } from "../../../collection/bedsheets/page";
import { curtainsProducts } from "../../../collection/curtains/page";
import { pillowCoversProducts } from "../../../collection/pillow-covers/page";
import { sofaCoversProducts } from "../../../collection/sofa-covers/page";

const categoryDataMap: Record<string, any[]> = {
  "bedsheets": bedsheetsProducts.map(p => ({ ...p, categoryName: "Premium Bedsheets" })),
  "curtains": curtainsProducts.map(p => ({ ...p, categoryName: "Elegant Curtains" })),
  "pillow-covers": pillowCoversProducts.map(p => ({ ...p, categoryName: "Pillow Covers" })),
  "sofa-covers": sofaCoversProducts.map(p => ({ ...p, categoryName: "Sofa Covers" }))
};

function ProductContent({ params }: { params: { category: string, slug: string } }) {
  const categoryProducts = categoryDataMap[params.category];
  ...
  const product = categoryProducts.find(
    (p: any) => p.name.toLowerCase().replace(/[^a-z0-9]+/g, '-') === params.slug
  );
...
export default function CategoryProductPage({ params }: { params: { category: string, slug: string } }) {
  return (
